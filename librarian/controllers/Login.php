...
$emailCheck = $connection->prepare('SELECT COUNT(1) FROM librarian WHERE email LIKE :email AND account_activated = 1');
...
if ($emailCheck->fetchColumn() > 0)
{
    ...
    $password_hash = $connection->prepare('SELECT librarian_id, user_name, password_hash FROM librarian WHERE email LIKE :email');
    ...
    $user_id =  $password_hash["librarian_id"];
